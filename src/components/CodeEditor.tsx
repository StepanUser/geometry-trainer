...
export const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  useEffect(() => {
    ...
    loader.init().then((monaco) => {
      ...
      const editor = monaco.editor.create(current, {model: model, ...editorSettings});
  ...
  const handleRunCode = () => {
    if (monacoRef.current) {
      ...
      const jsonDataWrapper = `
        const jsonData = {
          ${jsonObjects.map((json, index) => `json${index}: ${json}`).join(',\n')},
          getAll: function() { return [${jsonObjects.map((_, index) => `this.json${index}`).join(', ')}]; }
        };
      `;
